...
class BatchRenameApp:
    ...
    def _apply_runtime_theme(self) -> None:
        ...
        for shadow_name in ("settings_panel_shadow", "templates_panel_shadow"):
            shadow = getattr(self, shadow_name, None)
            if shadow is not None:
                shadow.configure(background=colors["panel_shadow"])
        history_window = self.dialogs.windows.get("history-center")
        if history_window is not None and history_window.winfo_exists():
            history_window.configure(background=colors["background"])
    ...
    def _show_about(self) -> None:
        def build(window: tk.Toplevel) -> None:
            ...
            content = (
                ...
                "• 本地操作日志、规则查询、逐项执行与撤回状态\n"
                "• 撤回管理、整批安全检查、嵌套目录逆序恢复与失败重试\n\n"
                "后续方向\n"
                "• 日志导出与更丰富的历史统计\n\n"
...
5. 点击“确认执行”，核对汇总并二次确认。程序会先建立本地操作档案，再逐项修改并保存结果；无法建立日志时不会开始改名。
6. 层级、文件夹/文件范围和扩展名保护位于左下角“设置”；正则模板与设置互斥显示，再次点击、按 Esc 或点击结果区会收起。

操作日志与撤回管理

从“功能 → 操作日志”可按关键词和状态查询历史操作，查看根目录、规则、范围、执行统计以及每一项的原路径、新路径、执行结果和撤回状态。档案保存在当前用户的 %LOCALAPPDATA%\BatchRename\operations 目录，程序重启后仍可读取。

从“功能 → 撤回管理”选择操作后，必须先执行整批安全检查。程序会确认当前名称仍存在、类型没有变化且原名称未被其他项目占用；任一项目存在风险时整批撤回都不会开始。检查通过后才能确认撤回，文件夹与内部项目会按安全顺序恢复。运行中遇到占用或权限错误会立即停止，已恢复和待重试项目分别记录在日志中。
...
正则模板使用约 40% 的模板库和 60% 的规则详情双栏布局，完整展示用途、表达式、替换内容、处理前后示例和必要选项；设置按扫描范围、处理对象和名称保护三组排列。面板从入口附近打开，带边框和阴影，可拖动标题区域调整位置，也可点击“关闭”或按 Esc 收起。
...
撤回依赖操作日志和当前磁盘状态，不能代替备份。重要目录仍应先备份，并认真检查预览、日志和安全检查结果。"""
