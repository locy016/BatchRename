...
| 默认窗口 | 跟随鼠标所在显示器自动取合适尺寸，完整工作台最小保持 1120×720 |
...
标准和舒展布局在左侧按照真实操作顺序放置目录、查找规则、扫描、替换内容、结果预览和确认执行；右侧从上到下依次呈现目录概况、统一结果表、匹配统计和处理进度。目录概况会显示当前目录、扫描范围、文件夹总数和文件总数，让使用者先确认程序实际读到了哪里、读到了多少内容。窗口放大后，新增空间会优先交给结果列表，不会在设置区域留下大片空白。

![批量重命名紧凑界面：深色导航条与展开的工作流抽屉](docs/images/batch-rename-compact.png)

竖屏或窗口内容比例偏窄时，完整流程自动收进 64 像素导航条。点击顶部流程图标会在当前窗口内展开抽屉；正则模板和设置位于导航条底部，三类浮层互斥显示。调整窗口或切换外观前已经填写的目录、规则、匹配快照和预览结果都会保留。

文件夹和文件位于同一张表中，列顺序为“类型、所在目录、原名称、新名称、状态、说明”。所在目录只显示当前根目录之后的部分，直属项目显示“（根目录）”，避免每行重复冗长的绝对路径。原名称和新名称会随结果区宽度自动伸缩，新名称使用当前主题的靛蓝强调色；文件夹优先显示，同类项目按名称中的数字自然排序，因此 `文件2` 会排在 `文件10` 前面。

类型、状态和说明采用形状与颜色共同表达的紧凑图标。文件夹与文件使用不同轮廓；绿色勾表示可修改，橙色横线表示名称未变化，红色警示表示存在阻止原因。鼠标停在图标上会显示完整文字，点击状态或说明图标会在主窗口所在屏幕中央打开“匹配结果详情”，集中展示相对目录、原名称、新名称、状态和完整说明。
...
点击左下角“正则模板”会在主窗口内打开工作面板。面板把约 40% 的空间交给分类与模板库，把约 60% 的空间交给规则详情；模板按日期时间、编号整理、标签清理、文本清理、片段调整和扩展名分类，每条规则都展示用途、查找表达式、替换内容、处理前后效果和必要选项。
...
结果表上方的目录概况描述扫描范围本身：当前根目录、全部层级或指定层级、扫描所得文件夹总数和文件总数，以及无法读取位置的数量。目录总量在同一次遍历中得到，不会因为当前只勾选文件夹或文件而失真。

匹配统计固定放在结果表下方，使用四种明确口径：
...
预览上限默认 100 条，只限制表格显示数量，不改变完整统计和最终执行范围。类型、状态和说明图标均提供完整悬浮提示；状态与说明还可以点击查看单项详情。长名称或相对目录被列宽缩短时，把鼠标停在相应单元格即可查看全文；横向滚动条仅在内容确实无法容纳时出现。

## 扫描范围与设置

左下角“设置”以完整工作面板展示三组影响扫描和预览的选项：扫描范围、处理对象和名称保护。
...
“跟随系统”会读取 Windows 的应用外观，并在窗口重新获得焦点时再次检查；“浅色”和“深色”用于固定当前选择。外观设置保存在当前用户的本地配置中，下次启动会自动恢复。主题切换会同步更新菜单、侧栏、结果表、语义图标、悬浮提示、滚动条、输入控件和工作面板，不会触发重新扫描，也不会清空当前结果。

### 多分辨率自适应

应用启动时读取鼠标指针所在显示器的可用工作区，再按屏幕比例选择初始尺寸：

- 4:3、16:10 和 16:9 标准屏优先使用工作区宽高的 50%，并保证不小于 1120×720。
- 21:9、32:9 等超宽屏适当提高窗口高度并限制内容宽高比，避免表格被无意义拉长。
- 竖屏优先保留可用宽度，限制窗口高度比例，并直接进入紧凑工作台。

窗口内部按当前尺寸实时切换三档布局：竖屏和特殊窄比例使用紧凑导航，1120–1439 的横向窗口使用标准模式，1440 及以上使用舒展模式。缩放采用短延时平滑重排；结果表始终保留全部安全信息，三列语义图标维持固定窄宽度，所在目录、原名称和新名称分享其余空间。
...
- 扫描匹配与结果预览分离，不重复读取目录。
- 文件夹与文件统一列表、自然排序、相对目录和新名称强调。
- 类型、状态、说明语义图标，完整悬浮提示和单项详情窗口。
- 名称未变化、目标冲突、重复目标和非法名称解释。
- 二次确认、逐项进度、结果详情和多屏窗口管理。
- 标准屏、超宽屏和竖屏初始尺寸适配，1120×720 完整工作台下限。
- 紧凑、标准、舒展三档工作台及状态保留式缩放。
- 当前目录、扫描范围、文件夹与文件总量组成的目录概况。
- 位于结果表下方的匹配、可修改、名称未变化和阻止执行统计。
- 跟随系统、浅色、深色三态外观及本地保存。
- 40% / 60% 双栏正则工作面板和三分组设置面板。
