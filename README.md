...
| 默认窗口 | 跟随鼠标所在显示器自动取合适尺寸，最小保持 960×680 |
...
窗口较窄或使用竖屏时，完整流程自动收进 64 像素导航条。点击顶部流程图标会在当前窗口内展开抽屉；正则模板和设置位于导航条底部，三类浮层互斥显示。调整窗口前已经填写的目录、规则、匹配快照和预览结果都会保留。
...
- 4:3、16:10 和 16:9 标准屏优先使用工作区宽高的 50%，并保证不小于 960×680。
- 21:9、32:9 等超宽屏适当提高窗口高度并限制内容宽高比，避免表格被无意义拉长。
- 竖屏优先保留可用宽度，限制窗口高度比例，并直接进入紧凑工作台。

窗口内部按当前宽度实时切换三档布局：960–1119 为紧凑模式，1120–1439 为标准模式，1440 及以上为舒展模式。缩放采用短延时平滑重排；结果表始终保留类型、原名称、新名称和状态，路径与说明会根据空间伸缩，完整内容仍可通过鼠标悬浮查看。
