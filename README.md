...
标准和舒展布局在左侧按照真实操作顺序放置目录、查找规则、扫描、替换内容、结果预览和确认执行；右侧集中展示匹配统计、统一结果表和处理进度。窗口放大后，新增空间会优先交给结果列表，不会在设置区域留下大片空白。

![批量重命名紧凑界面：深色导航条与展开的工作流抽屉](docs/images/batch-rename-compact.png)

竖屏或窗口内容比例偏窄时，完整流程自动收进 64 像素导航条。点击顶部流程图标会在当前窗口内展开抽屉；正则模板和设置位于导航条底部，三类浮层互斥显示。调整窗口前已经填写的目录、规则、匹配快照和预览结果都会保留。

文件夹和文件位于同一张表中，列顺序为“类型、所在目录、原名称、新名称、状态、说明”。所在目录只显示当前根目录之后的部分，直属项目显示“（根目录）”，避免每行重复冗长的绝对路径。原名称和新名称会随结果区宽度自动伸缩，新名称继续使用青绿色强调；文件夹优先显示，同类项目按名称中的数字自然排序，因此 `文件2` 会排在 `文件10` 前面。
