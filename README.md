<div align="center">
  <img src="src-tauri/icons/128x128.png" width="112" alt="文件名管理工具图标">
  <h1>文件名管理</h1>
...
文件名管理是一款面向 Windows 的文件名管理工具，覆盖文件名称管理、批量重命名、结果预览、操作日志与整批撤回。2.0 采用 Rust、Tauri 2 和 Vue 3 构建，在保持既有安全语义与日志格式兼容的基础上，让大目录扫描、结果浏览和操作管理保持流畅。当前版本为 `2.0.0-alpha.1` 测试候选。
...
- 结果显示数量可从 1 条调整到全部；默认显示 100 条，仅影响表格，完整统计和最终执行范围始终保持一致。
...
2.0 已完成对原 Python 版本公开能力、安全规则和 `schema_version = 1` 操作档案的兼容验证。旧实现已退出发行与维护范围；仓库继续保留中立规则夹具、历史日志样本、性能结论和迁移审计，后续开发只围绕 Rust、Tauri 与 Vue 版本进行。
